...
function buildHeaderTools() {
  ...
  // A page's own actions (Save this list on Home) are written into the header by the
  // builder and sit beside Refresh, never at the foot of the page.
  ...
  refresh.type = 'button';
  refresh.append(icon(ICONS.refresh, 16), document.createTextNode('Read the boards again'));
  refresh.addEventListener('click', () => location.reload());

  const tools = create('div', 'header-tools');
  tools.append(...actions, refresh);
  header.append(tools);
}
...
function buildFooter() {
  ...
  main.append(create('p', 'page-foot', 'HSG · SAST · read-only. Every figure here is read from the board it belongs to.'));
}
