...
- **The Home banner** — the navy banner above the list, and the only place Home says its two totals: how many things need a decision (the figure and the sentence) and how many are only waiting (the line under it). It does not name the boards that answered (the readings line does) and it has no button, since the menu already goes to The boards. When nothing is waiting it turns green rather than disappearing. There are no figure tiles on Home.
- **The group caveat** — each group heading in **By what it needs** carries an (i), the same button and `.tile-about` note a figure card uses (`aria-expanded`, `aria-controls`, printed open), saying what the group counts and what it does not.
