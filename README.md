...
| Page | What it does |
| --- | --- |
| `pages/home.html` | The landing page: everything the three boards say needs attention, pulled into one list. **Needs a decision** first, then **Waiting on someone**, each row saying how many, what it is and which board it came from, and each row a link straight to that group on that board. Switch it to **By board** to read it a board at a time. **Save this list**, in the page header beside **Read the boards again**, writes the whole thing to a spreadsheet file for a meeting. Above the list, a line naming when each board was read and which did not answer, because two boards read on different days are not one reading, and the navy banner with the two totals: how many things need a decision and how many are only waiting. Each total is said once, in the banner: the list groups carry no count of their own, and each group heading has an (i) saying what it counts and what it does not. **Saved views** sits behind one button at the end of the filter row and opens by itself when the page is showing a saved view. |
| `pages/boards.html` | Three cards, one per board. The whole card is the way in. Each shows that board's headline figures, how many things are waiting on it and when it was read. What each board holds, and who opens it, is the table underneath. |
